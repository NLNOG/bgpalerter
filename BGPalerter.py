...
def check_stats():
    ...
    for key, value in stats.items():
        print(value)
        if len(value["peers"]) > config.get("number-peers-before-hijack-alert", 0):
...
def low_visibility_message(data):
    message = "The prefix " + data["prefix"] + " is not visible anymore from " + data["lost-peers"] + " peers"
    send_to_slack(message)

def run_service(config):
    ris = RisListener(config)
    ris.on("hijack", collect_stats)
    ris.on("low-visibility", low_visibility_message)
...
#     run_service(config)
#
# if not self.visibility_index[str_prefix]:
#     self.visibility_index[str_prefix] = {}
#
# self.visibility_index[str_prefix][peer] = True
#
# self._detect_low_visibility(str_prefix)
